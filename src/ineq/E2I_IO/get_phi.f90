!phi_i(a_,a_)
subroutine ed_get_phisc_n1(self,Nlat,iorb,arg)
  ...
  if(present(arg))arg = argsc_ineq(:,iorb,iorb)
end subroutine ed_get_phisc_n1


!phi_i,ab
subroutine ed_get_phisc_n3(self,Nlat,arg)
  real(8),dimension(:,:,:)          :: self
  integer                           :: Nlat
  real(8),dimension(:,:,:),optional :: arg
  if(.not.allocated(phisc_ineq))stop "ed_get_phisc error: phisc_ineq not allocated"
