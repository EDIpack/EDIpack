...
contains
  ...
  function g0and_general_superc(a) result(G0and)
    ...
    do i=1,Ldelta
       ...
       FGorb(      :Norb,Norb+1:    ) =   -impHloc_anomalous(1,1,:,:)        -       nn2so_reshape(Delta(2,:,:,:,:,i), Nspin,Norb)
       FGorb(Norb+1:    ,      :Norb) =   -conjg(impHloc_anomalous(1,1,:,:)) - conjg(nn2so_reshape(Delta(2,:,:,:,:,i), Nspin,Norb))!technically this is not conjg but being real who cares
